...
这里作者也做出了相应改进:1. Generator有很多层，在每一层都计算NCE可以得到更好的效果; 2. 在计算key的时候，类似于MOCO的设计，要将梯度停掉；3. Identity Loss: 类似CycleGAN，对于已经是目标域的图像，Generator不应该再对其做出改变；4. External NCE Loss: 把其他图像也视为负样本。
