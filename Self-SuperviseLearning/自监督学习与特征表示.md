...
这篇论文是对上一篇13.论文的拓展，整了两个CUT，也就是两个Generator在源域和目标域之间互相转换，然后把两个Generator的Encoder拿出来过两层MLP后做 Info NCE Loss。这里除了GAN Loss，作者还搞了一个 SIM Loss来辅助Discriminator对生成图像做判别，计算两个Latent feature的相似程度，避免model collapse。
